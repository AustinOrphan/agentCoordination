...
class LifecycleManager:
    ...
    def __init__(self, config_file: str = "agent_config.json", 
        ...
        self.last_start_time: Dict[str, datetime] = {}  # Track last start time to prevent rapid restarts
    ...
    def start_agent(self, agent_id: str) -> bool:
        ...
        # Additional check - look for recently started processes
        lifecycle_file = self.communication_dir / agent_id / "status" / "lifecycle.json"
        if lifecycle_file.exists():
            try:
                with open(lifecycle_file, 'r') as f:
                    lifecycle = json.load(f)
                if lifecycle.get("status") == "running":
                    # Check if the status is very recent (within last 30 seconds)
                    last_update = lifecycle.get("last_update", "")
                    if last_update:
                        try:
                            update_time = datetime.fromisoformat(last_update.replace("Z", "+00:00"))
                            if (datetime.now(timezone.utc) - update_time).seconds < 30:
                                logger.info(f"Agent {agent_id} was recently started, skipping restart")
                                return True
                        except Exception:
                            pass
            except Exception as e:
                logger.warning(f"Error reading lifecycle file for {agent_id}: {e}")
        
        # Check cooldown period to prevent rapid restarts
        cooldown_seconds = 30  # Don't restart agents more than once every 30 seconds
        if agent_id in self.last_start_time:
            ...
            if time_since_last_start < cooldown_seconds:
                logger.info(f"Agent {agent_id} in cooldown period ({time_since_last_start:.1f}s), skipping restart")
                return False
        ...
        try:
            ...
            self.last_start_time[agent_id] = datetime.now()  # Record start time for cooldown
            ...
            if heartbeat_file.exists():
                try:
                    ...
                    if not last_activity_str:
                        # Skip heartbeat check if no last_activity timestamp
                        continue
